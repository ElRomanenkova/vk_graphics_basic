...
float linear_depth(vec2 coord, float depth)
{
  vec4 posScaled = vec4(coord, depth, 1.0f);
  vec3 normCoords = (Params.lightMatrix * posScaled).xyz / (Params.lightMatrix * posScaled).w;
  float d1 = texture(shadowMap, normCoords.xy * 0.5f).x;
  float d2 = normCoords.z;
  return abs(d1 - d2);
}
...
void main()
{
  ...
  const vec4 albedo = vec4(params.objColor, 1.0f);
  out_fragColor   = lightColor * shadow * albedo;

  if (Params.isSss)
  {
    const float linearShadowMapDepth = linear_depth(shadowTexCoord, shadowDepth);

    const float s = linearShadowMapDepth / Params.sssScale;
