...
void main()
{
  ...
  const float lightCos  = dot(normalize(Params.illuminantDirection), -lightDir);
  const float intensity = smoothstep(Params.illuminantOuterCos, Params.illuminantInnerCos, lightCos);
