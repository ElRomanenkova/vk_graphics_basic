...
void SimpleShadowmapRender::AllocateResources()
{
  ...
  matrices = m_context->createBuffer(etna::Buffer::CreateInfo{
         .size        = sizeof(LiteMath::float4x4) * m_numInstances,
         .bufferUsage = vk::BufferUsageFlagBits::eStorageBuffer,
         .memoryUsage = VMA_MEMORY_USAGE_CPU_ONLY });

  m_matrices = matrices.map();
  ...
  m_instanceVisibleCount = instanceVisibleCount.map();

  visibleInstances = m_context->createBuffer(etna::Buffer::CreateInfo{
         .size        = sizeof(uint32_t) * m_numInstances,
         .bufferUsage = vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eTransferDst,
         .memoryUsage = VMA_MEMORY_USAGE_CPU_ONLY });

  m_visibleInstances = visibleInstances.map();
  ...
  m_indirectBuffer = indirectBuffer.map();
}
...
void SimpleShadowmapRender::PreparePipelines()
{
  ...
  m_pFSQuad->Create(m_context->getDevice(), 
    ...
    vk_utils::RenderTargetInfo2D{ 
      ...
      m_swapchain.GetFormat(),                    // this is debug full scree quad
...
void SimpleShadowmapRender::SetupSimplePipeline()
{
  ...
  m_pBindings->BindBuffer(0, matrices.get());
  m_pBindings->BindBuffer(1, instanceVisibleCount.get());
  m_pBindings->BindBuffer(2, visibleInstances.get());
  ...
  m_computePipeline = pipelineManager.createComputePipeline("frustum_culling", {});
...
void SimpleShadowmapRender::RunComputeShader(VkCommandBuffer a_cmdBuff, const float4x4 &a_wvp)
{
  VkShaderStageFlags stageFlags = (VK_SHADER_STAGE_COMPUTE_BIT);

  std::vector<LiteMath::float4x4> instances;
  instances.reserve(m_numInstances);

  std::mt19937 gen(10);
  std::uniform_real_distribution<float> dist(-1000.f, 1000.f);

  for (size_t i = 1; i <= m_numInstances; i++)
  {
    auto x = dist(gen);
    auto y = 0.0f;
    auto z = dist(gen);

    auto new_pos = LiteMath::translate4x4({ x, y, z });
    instances.push_back(new_pos);
  }

  memcpy(m_matrices, instances.data(), sizeof(LiteMath::float4x4) * instances.size());

  vkCmdFillBuffer(a_cmdBuff, instanceVisibleCount.get(), 0, VK_WHOLE_SIZE, 0);
  vkCmdFillBuffer(a_cmdBuff, visibleInstances.get(), 0, VK_WHOLE_SIZE, 0);
  pushConst2Comp.projView = a_wvp;

  for (uint32_t i = 0; i < m_pScnMgr->InstancesNum(); i++)
  {
    pushConst2Comp.box = m_pScnMgr->GetInstanceBbox(i);
    pushConst2Comp.instNumber = m_numInstances;
    vkCmdPushConstants(a_cmdBuff, m_computePipeline.getVkPipelineLayout(), stageFlags, 0, sizeof(pushConst2Comp), &pushConst2Comp);
...
void SimpleShadowmapRender::DrawSceneCmd(VkCommandBuffer a_cmdBuff, const float4x4 &a_wvp)
{
  ...
  VkDrawIndexedIndirectCommand *indirBufferCmd = (VkDrawIndexedIndirectCommand *)m_indirectBuffer;
  ...
  indirBufferCmd->firstIndex       = mesh_info.m_indexOffset;
  indirBufferCmd->firstInstance    = 0;
  indirBufferCmd->indexCount       = mesh_info.m_indNum;
  indirBufferCmd->instanceCount    = *(uint *)m_instanceVisibleCount;
  indirBufferCmd->vertexOffset     = mesh_info.m_vertexOffset;

  vkCmdPushConstants(a_cmdBuff, m_basicForwardPipeline.getVkPipelineLayout(), stageFlags, 0, sizeof(pushConst2M), &pushConst2M);

  vkCmdDrawIndexedIndirect(a_cmdBuff, indirectBuffer.get(), 0, 1, 0);
} 
...
void SimpleShadowmapRender::BuildCommandBufferSimple(VkCommandBuffer a_cmdBuff, VkImage a_targetImage, VkImageView a_targetImageView)
{
  ...
  //
  {
    ...
    { 
      ...
      auto set = etna::create_descriptor_set(simpleShadowInfo.getDescriptorLayoutId(0), { 
        etna::Binding {2, vk::DescriptorBufferInfo {matrices.get(), 0, VK_WHOLE_SIZE } }, 
        etna::Binding {3, vk::DescriptorBufferInfo {visibleInstances.get(), 0, VK_WHOLE_SIZE } } });
  ...
  //
  {
    ...
    auto set = etna::create_descriptor_set(frustumCullingInfo.getDescriptorLayoutId(0), { 
      etna::Binding {0, vk::DescriptorBufferInfo {matrices.get(), 0, VK_WHOLE_SIZE } }, 
      etna::Binding {1, vk::DescriptorBufferInfo {instanceVisibleCount.get(), 0, VK_WHOLE_SIZE } },
      etna::Binding {2, vk::DescriptorBufferInfo {visibleInstances.get(), 0, VK_WHOLE_SIZE } } });

    VkDescriptorSet vkSet = set.getVkSet();

    vkCmdBindDescriptorSets(a_cmdBuff, VK_PIPELINE_BIND_POINT_COMPUTE, m_computePipeline.getVkPipelineLayout(), 0, 1, &vkSet, 0, VK_NULL_HANDLE);
    vkCmdBindPipeline(a_cmdBuff, VK_PIPELINE_BIND_POINT_COMPUTE, m_computePipeline.getVkPipeline());
    
    RunComputeShader(a_cmdBuff, m_worldViewProj);
  }
