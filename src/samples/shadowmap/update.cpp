...
void SimpleShadowmapRender::UpdateUniformBuffer(float a_time)
{
  ...
  m_uniforms.illuminantDirection = m_light.cam.forward();
  m_uniforms.illuminantInnerCos  = cos(illuminantInner  * M_PI / 180);
  m_uniforms.illuminantOuterCos = cos(illuminantOuter * M_PI / 180);
